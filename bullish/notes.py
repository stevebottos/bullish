    ...
    if os.path.exists(constants.Files.NOTES):
        with open(constants.Files.NOTES, "r") as f:
            ...
            for ticker in notes:
                ...
                print(
                    yaml.dump(
                        notes[ticker], allow_unicode=True, default_flow_style=False
                    )
                )
    else:
        print(
            "You don't have any notes yet! To get started, use \"bullish watchlist add <ticker>\" to create a watchlist. You'll then be able to start adding notes for that ticker."
        )
...
def update(ticker, field):
    ...
    with open(constants.Files.NOTES, "r+") as f:
        ...
        if ticker not in notes:
            print(
                f'{ticker} is not in your watchlist. Use "bullish watchlist add <ticker>" to add it.'
            )
            return
        ...
        if field not in notes[ticker]:
            ...
            print(
                f"{field} is not an editable field. Choose from:\n\t{acceptable_fields}"
            )
            return

        time = str(datetime.now())
        note = input(time + ": ")
        notes[ticker][field][time] = note
